...
async function main() {
  ...
  const polymarketMarkets = await polymarket.fetchMarkets({
    limit: 5,
  ...
  console.log(
    'polymarket fetchMarkets params:' + JSON.stringify({
      limit: 5,
      minVolume: 30_000_000,
      minLiquidity: 2_000_000,
    })
  ...
  // Limitless fetchMarkets input (e.g. sortBy: 'high_value' | 'volume' | ...)
  const limitlessSortBy = 'high_value';

  console.log('\nFetching markets...');
  const limitlessMarkets = await limitless.fetchMarkets({
    limit: 5,
    sortBy: limitlessSortBy,
  });
